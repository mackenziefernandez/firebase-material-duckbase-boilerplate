...
export function getFirebaseConnection() {
  firebaseApp = firebaseApp ? firebaseApp : initialize();
  return firebaseApp;
}

function initialize() {
  return firebase.initializeApp(env.firebaseConfig);
}

export const firebaseAuth = getFirebaseConnection().auth();
export const firebaseDb = getFirebaseConnection().database();
